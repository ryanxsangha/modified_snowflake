...
// New Implementation: Goal -> record server-side ASN
// Approach: Use IP data as input to CAIDA prefix2as database to find ASN
// RecordASN finds the ASN for the server's IP address using the CAIDA prefix-to-AS dataset.
func (l *SnowflakeListener) RecordASN(datasetPath string) (string, error) {
	// Get the server's IP address.
	serverIP, _, err := net.SplitHostPort(l.Addr().String())
	if err != nil {
		return "", errors.New("failed to parse server address")
	}
	...
	// Open the CAIDA prefix-to-AS dataset file.
	file, err := os.Open(datasetPath)
	if err != nil {
		return "", errors.New("failed to open dataset file")
	}
	defer file.Close()

	// Parse the dataset to find the matching ASN.
	...
	for scanner.Scan() {
		...
		prefix := fields[0] // e.g., "192.0.2.0/24"
		asn := fields[1]    // e.g., "12345"
		...
		// Check if the server's IP belongs to this CIDR range.
		if cidr.Contains(ip) {
			return asn, nil
		}
	}
	...
	if err := scanner.Err(); err != nil {
		return "", errors.New("error reading dataset file")
	}

	return "", errors.New("ASN not found for the server's IP address")
}
